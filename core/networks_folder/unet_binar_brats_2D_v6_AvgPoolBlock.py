...
'''
* prin aceasta arhitectura se propune explorarea varierii dimensiunii nucleului din AvgPooling, la mecanismul de atentii
...
class Squeeze_and_extraction_block(nn.Module):
    def __init__(self, feature):
        ...
        self.compute_channel_weights = nn.Sequential(
            ...
            nn.AdaptiveAvgPool2d((2, 2)), # S-A REZOLVAT! 1x1 este DE FAPT, DIMENSIUNEA DE IESIRE
            nn.Flatten(),

            # Primul strat FC
            #nn.Linear(2*feature, 2*feature),
            nn.Linear(feature//11+1, 2*feature),
...
class UNET_semantic_brats_2D_v6(nn.Module):
    def __init__(self, in_channels=4, out_channels=3, features = [64, 128, 256, ]):
        ...
        '''
        super(UNET_semantic_brats_2D_v6, self).__init__()

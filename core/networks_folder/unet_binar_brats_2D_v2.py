...
UNET_binar_brats_2D_v1 - DAR cu triple conv, in loc de DoubleConv
...
class Upsample(nn.Module):
    def __init__(self, feature):
        ...
        self.Upsample = nn.Sequential(
            ...
            #nn.Conv2d(feature*4, feature, kernel_size=3, stride=1, padding = 'same')
...
class UNET_binar_brats_2D_v2(nn.Module):
    def __init__(self, in_channels=4, out_channels=1, features = [64, 128, 256, ]):
        ...
        '''
        super(UNET_binar_brats_2D_v2, self).__init__()
        ...
        for feature in features:
            self.downs.append(TripleConv(feature, feature)) # inputul etse in_channels, iar la output e cat specifica feature
        ...
        for feature in reversed(features):
            self.ups_conv.append(TripleConv(2*feature, feature))

        # spatiul latent
        self.bottleneck = TripleConv(features[-1]*2, features[-1]*2)

...
- aici se incearca o ALTA ARHITECTUA PENTRU ATENTII - se scoate un strat dense
...
class UNET_binar_brats_2D_v4(nn.Module):
    def __init__(self, in_channels=4, out_channels=1, features = [64, 128, 256, ]):
        ...
        '''
        super(UNET_binar_brats_2D_v4, self).__init__()
    ...
    def forward(self, x):
        ...
        for i in range(0, len(self.ups_expand)): # se merge cu PASUL DE 2 deoarece syunt 2 convolutii
            ...
            channedl_weights = F.softmax(channedl_weights) # il pun aici pentyru ca nu pot inainutrul blocului

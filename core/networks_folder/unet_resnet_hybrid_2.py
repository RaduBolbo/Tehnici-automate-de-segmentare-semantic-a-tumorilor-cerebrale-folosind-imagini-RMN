...
class UNET_RESNET_HYBRID_2(nn.Module):
    def __init__(self, in_channels=4, out_channels=3, features = [64, 128, 256, ]):
        ...
        '''
        super(UNET_RESNET_HYBRID_2, self).__init__()
        ...
        number_of_blocks = 3
        number_of_blocks_list = []
        for feature in features:
            number_of_blocks_list.append(number_of_blocks)
            ...
            self.downs.append(MiniResNet(feature, feature, number_of_blocks))
        ...
        i = 0
        number_of_blocks_list_reversed = list(reversed(number_of_blocks_list))
        for feature in reversed(features):
            #self.ups_conv.append(DoubleConv(feature, feature))
            self.ups_conv.append(MiniResNet(feature, feature, number_of_blocks_list_reversed[i]))
            i += 1

...
'''
* aceasta varainvta v4 propune operatia de adunarer in locul concatenarii, pentru skip connecti8on-urile de peste DoubleConv
...
class Adapt_to_expected_no_classes(nn.Module):
    def __init__(self, feature):
        super(Adapt_to_expected_no_classes, self).__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(2*feature, feature, kernel_size=1, stride=1, padding = 'same'),
        )
    def forward(self, x):
        return self.conv(x)
...
class UNET_semantic_brats_2D_v4(nn.Module):
    def __init__(self, in_channels=4, out_channels=3, features = [64, 128, 256, ]):
        ...
        '''
        super(UNET_semantic_brats_2D_v4, self).__init__()
        ...
        self.adapt_to_no_classes = nn.ModuleList()
        ...
        for feature in reversed(features):
            ...
            self.adapt_to_no_classes.append(
                Adapt_to_expected_no_classes(feature)
            )
    ...
    def forward(self, x):
        ...
        for down, skrinker in zip(self.downs, self.skrinker_conv):
            ...
            # NU MAI FAC concatenare, ci suma
            #x = torch.cat((x, short_skip_connections.pop(0)), dim = 1)
            x = x + short_skip_connections.pop(0)
            #print(x.shape, '4')
        ...
        for i in range(0, len(self.ups_expand)): # se merge cu PASUL DE 2 deoarece syunt 2 convolutii
            ...
            x = self.adapt_to_no_classes[i](x)

            ##################### MECANISMUL DE ATENTII PER CANAL #########################

            short_skip_connections.append(x)
            #print(x.shape, 'a')
            #print(short_skip_connections[-1].shape, 'a')
            x = self.ups_conv[i](x)
            #print(x.shape, 'a')

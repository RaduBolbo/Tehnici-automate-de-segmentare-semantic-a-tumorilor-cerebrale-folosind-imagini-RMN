...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, DIR_PATH, normalize, zoom=False, rotate=False, horiz_flip=False, vert_flip=False, gauss_disp=0, returnpath=False):
        secondary_paths = []
    ...
    def __getitem__(self, index):
        ...
        x_t1 = cv2.imread(self.secondary_paths[index] + '\\' + 't1.png', cv2.IMREAD_ANYDEPTH)
        x_t1ce = cv2.imread(self.secondary_paths[index] + '\\' + 't1ce.png', cv2.IMREAD_ANYDEPTH)
        x_t2 = cv2.imread(self.secondary_paths[index] + '\\' + 't2.png', cv2.IMREAD_ANYDEPTH)
        x_flair = cv2.imread(self.secondary_paths[index] + '\\' + 'flair.png', cv2.IMREAD_ANYDEPTH)
        y = cv2.imread(self.secondary_paths[index] + '\\' + 'seg.png', cv2.IMREAD_ANYDEPTH)
        #print(x_t1.dtype)
        ##print(np.unique(y))
        #print(self.secondary_paths[index])
        ...
        if self.zoom:
            ...
            if zoom_factor_x < 0.25 and zoom_factor_x > 0.05 and zoom_factor_y < 0.25 and zoom_factor_y > 0.05:
                ...
                y = cv2.resize(y, (240, 240), interpolation = cv2.INTER_NEAREST) # se impune ca label-urile sa aiab nearest. Altfel, apar clase "intermediare"
        ...
        if x_t1.dtype == np.uint8:
            #print('tipul de date e uint8')
            #print(np.min(x_t1), np.max(x_t1))
            #print(np.min(x_t1ce), np.max(x_t1ce))
            #print(np.min(x_t2), np.max(x_t2))
            #print(np.min(x_flair), np.max(x_flair))
            '''
            ...
            '''
            x_t1 = np.double(x_t1[:,:] / np.max(x_t1[:,:]))
            x_t1ce = np.double(x_t1ce[:,:] / np.max(x_t1ce[:,:]))
            x_t2 = np.double(x_t2[:,:] / np.max(x_t2[:,:]))
            x_flair = np.double(x_flair[:,:] / np.max(x_flair[:,:]))
            ##print(x_t1.shape)
        elif x_t1.dtype == np.uint16:
            x_t1 = np.double(x_t1[:,:] / (2**16-1))
            x_t1ce = np.double(x_t1ce[:,:] / (2**16-1))
            x_t2 = np.double(x_t2[:,:] / (2**16-1))
            x_flair = np.double(x_flair[:,:] / (2**16-1))

        '''
        y = np.double(y[:,:,0]) # NU MAI impart la MAXIM, deoarece se face la LOSS selectia si se imparte canalul cu valori 0, 1, 2, 3, 4 in 4 canale.
        '''
        y = np.double(y[:,:])
        ...
        if self.returnpath:
            return x, y, self.secondary_paths[index]
        else:
            return x, y
    '''
    def afisare(self):
...
#################################################### UnlabeledDataset - se foloseste pentru setul oficial de validare ################################################################

class UnlabeledDataset(torch.utils.data.Dataset):

    def __init__(self, DIR_PATH, normalize, returnpath=False):
        secondary_paths = []
        for dir_name_2 in os.listdir(DIR_PATH):
            DIR_PATH_2 = DIR_PATH + '\\' + dir_name_2
            secondary_paths.append(DIR_PATH_2)
    ...
    def __getitem__(self, index):
        ...
        #y = cv2.imread(self.secondary_paths[index] + '\\' + 'seg.png')
        ##print(np.unique(y))
        #print(self.secondary_paths[index])
        ...
        ##print('dataloader returned y shape: ' + str(y.shape))
        if self.returnpath:
            return x, self.secondary_paths[index]
        else:
            return x
